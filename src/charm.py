...
class NginxIngressCharm(CharmBase):
    ...
    @property
    def _ingress_option(self) -> _ConfigOrRelation:
    ...
    def _validate(self) -> None:
        ...
        """
        if not is_backend_protocol_valid(self._ingress_option._backend_protocol):
            raise InvalidIngressOptionError(INVALID_BACKEND_PROTOCOL_MSG)
        hostnames = [self._ingress_option._service_hostname]
        hostnames.extend(self._ingress_option._additional_hostnames)
        for hostname in hostnames:
            if not invalid_hostname_check(hostname):
    ...
    def _namespace(self) -> Any:
        ...
        return self._ingress_option._namespace
    ...
    def _report_service_ips(self) -> List[str]:
        ...
        return [
            ...
            if x.metadata.name == self._ingress_option.k8s_service_name
        ]
    ...
    def _define_endpoint_slice(self) -> None:
        ...
        if self._ingress_option.use_endpoint_slice:
        ...
        for endpoint_slice in endpoint_slices.items:
            if endpoint_slice.metadata.name == self._ingress_option.k8s_endpoint_slice_name:
                continue
            else:
    ...
    def _define_service(self) -> None:
        ...
        for service in services.items:
            if service.metadata.name == self._ingress_option.k8s_service_name:
                continue
    ...
    def _define_ingress(self) -> None:
        ...
        for ingress in ingresses.items:
            if ingress.metadata.name == self._ingress_option.k8s_ingress_name:
                continue
