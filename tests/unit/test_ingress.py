...
def test_use_regex_config(k8s_stub: K8sStub, harness: Harness, ingress_relation):
    """
    arrange: set up test harness and ingress relation with use-regex config enabled.
    act: update the ingress relation with use-regex=True.
...
def test_use_regex_relation(k8s_stub: K8sStub, harness: Harness, ingress_relation):
    """
    arrange: set up test harness and ingress relation with use-regex relation enabled.
    act: update the ingress relation with use-regex=True.
    assert: use-regex=True and pathType=ImplementationSpecific.
    """
    harness.begin()
    app_data = ingress_relation.gen_example_app_data()
    app_data["use-regex"] = "true"
    app_data["strip-prefix"] = "true"
    ingress_relation.update_app_data(app_data)
    ingress_relation.update_unit_data(ingress_relation.gen_example_unit_data())
    harness.update_config({"service-hostname": "example.com"})
